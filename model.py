...
class ResNet(nn.Module):
    ...
    def forward(self, x):
      ...
      #  x4=F.upsample_bilinear(x4,scale_factor=4)
      #  x3=F.upsample_bilinear(x3, scale_factor=2)
      #  print(x4.size(),x3.size(),x2.size())
      #  x=torch.cat([x3,x4,x2],1)
        _,_,w,h=x4.size()

        x=F.avg_pool2d(x4,w).squeeze()
        x=self.fc(x)
